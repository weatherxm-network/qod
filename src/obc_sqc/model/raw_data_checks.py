...
class RawDataChecks:  # noqa: D101
    ...
    def raw_data_suspicious_check(  # noqa: PLR0915
        ...
        """This def detects faulty observations based on WMO criteria and annotates as faulty the observation which is
        unavailable. or has the greatest difference from the 10-minute median in case of a significant jump within a
        couple of raw data. As an example, if we have a dataset of temperatures [10,10,10.1,50,10.1,50,50,50,10.2,
        10.2,nan,10.2], then e.g., the difference between the 3rd and 4th elements is >2, but the difference of the
        ...
        the nan value will be annotated as faulty/invalid. It should be noted that the first 10min data are
        considered as invalid and no decision can be taken as median cannot be calculated. However,  if only 10min
        data are missing from an entire hour, rewards should be finally awarded
        ...
        if parameter != "wind_direction" and parameter != "precipitation_accumulated":  # noqa: PLR1714
            ...
            # if median cannot be calculated
            fnl_df["ann_no_median"] = 0
            ...
            # Check if 'median_diff_abs' is null
            no_median_mask = fnl_df["rolling_median"].isnull()  # noqa: PD003
            # Set 'raw_suspicious_anotation' to 1 for rows where 'median_diff_abs' is null
            fnl_df.loc[no_median_mask, "ann_no_median"] = ann_no_median
